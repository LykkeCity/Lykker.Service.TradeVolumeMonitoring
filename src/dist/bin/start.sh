...
EXECSTR="$JAVACMD \
    -DTradeVolumeMonitoringService -server -Dlog4j.configuration=file:///"$BASEDIR"/cfg/log4j.properties $JAVA_OPTS \
    ...
    -Dapp.home="$BASEDIR" \
    -Dbasedir="$BASEDIR" \
    -jar $BASEDIR/lib/trade.volume.monitoring.service-0.0.1.jar \
    $HTTP_CONFIG"
